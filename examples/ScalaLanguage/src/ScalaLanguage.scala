// Drag the block block around
// Shows how to use functions, traits, and animation in PulpCore and Scala.
...
class ScalaLanguage extends Scene2D {
    ...
    override def load = {
        ...
        leftPaddle.x.bindTo(() => Math.pow(leftPaddle.y - 240, 2) / 2000)
        leftPaddle.y.bindTo(() => Input.getMouseY)
        add(leftPaddle)

        // Timeline animation
        val rightPaddle = new ImageSprite("Block2.png", 640, 0)
        rightPaddle.setAnchor(Sprite.CENTER)
        val timelineY = Timeline(
            rightPaddle.y -> (0 to 480 dur 1500),
            rightPaddle.y -> (480 to 0 dur 1500 delay 1500))

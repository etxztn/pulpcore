...
//        assertTrue("Distribution not uniform: " + variance, variance >= 0 && variance < 1.25);
    ...
    @Test public void acos() {
        for (int f = -CoreMath.ONE; f <= CoreMath.ONE; f++) {
            double v = CoreMath.toDouble(f);
    ...
    @Test public void atan2NearZero() {
        for (int i = 0; i < NUM_TESTS; i++) {
            int fx = CoreMath.rand(-CoreMath.ONE*3/4, CoreMath.ONE*3/4);
            int fy = CoreMath.rand(-CoreMath.ONE*3/4, CoreMath.ONE*3/4);
    ...
    @Test public void atan2Any() {
        for (int i = 0; i < NUM_TESTS; i++) {
            double x = CoreMath.rand(CoreMath.MIN_DOUBLE_VALUE, CoreMath.MAX_DOUBLE_VALUE);
            double y = CoreMath.rand(CoreMath.MIN_DOUBLE_VALUE, CoreMath.MAX_DOUBLE_VALUE);
            int fx = CoreMath.toFixed(x);
            int fy = CoreMath.toFixed(y);
            double expectedResult = Math.atan2(y, x);
            double actualResult = CoreMath.toDouble(CoreMath.atan2(fy, fx));
            assertEquals("Bad result for " + x + ", " + y, expectedResult, actualResult, 0.001);
        }
    }
}
